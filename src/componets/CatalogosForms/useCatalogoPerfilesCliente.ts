...
const useCatalogoPerfilesCliente = (props: CatalogosGenericosProps) => {
    ...
    const tiposUsuario = useSelector(
        (state: StoreType) => {
            return state?.app?.catalogos?.['apm_tipo_usuarios']
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            id_tipo_usuario: Yup.string().required(intl.formatMessage({ id: 'input_validation_requerido' })),
        }),
    });
    ...
    useEffect(() => {
        ...
        if (props?.item && props?.item?.id_tipo) {
            setId_tipo_usuario(props?.item?.id_tipo+'');
            formik.setFieldValue("id_tipo_usuario", props?.item?.id_tipo+'');
        }
...
export default useCatalogoPerfilesCliente
