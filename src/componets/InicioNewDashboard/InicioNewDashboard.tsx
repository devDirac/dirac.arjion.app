...
import { InicioNewDashboardProps } from './types';
...
const InicioNewDashboard: React.FC<InicioNewDashboardProps> = (props: InicioNewDashboardProps) => {
    ...
    const handleOENE = async () => {
        try {
            setProcesando(true);
            const res = await getOENEHTTP(infoProyecto?.id_obra_principal, props?.usuario);
    ...
    const handlePendientesContabilizar = async () => {
        try {
            setProcesando(true);
            const res = await getPendientesContabilizarHTTP(infoProyecto?.id_obra_principal);
    ...
    const hanndleEstimacionesProceso = async () => {
        try {
            setProcesando(true);
            const res = await getEstimacionesProcesoHTTP(infoProyecto?.id_obra_principal);
    ...
    const hanndleFondoGarantia = async () => {
        try {
            setProcesando(true);
            const res = await getFondoGarantiaHTTP(infoProyecto?.id_obra_principal);
    ...
    const handleDeductivas = async () => {
        try {
            setProcesando(true);
            const res = await getDeductivasHTTP(props?.usuario);
    ...
    const handleAnticipos = async () => {
        try {
            setProcesando(true);
            const res = await getAnticiposHTTP(infoProyecto?.id_obra_principal);
    ...
    const handleAhorro = async () => {
        try {
            setProcesando(true);
            const res = await getAhorroHTTP(infoProyecto?.id_obra_principal);
    ...
    const handleOrdenCompra = async () => {
        try {
            setProcesando(true);
            const res = await getOrdenCompraHTTP(infoProyecto?.id_obra_principal);
    ...
    const handleReclasificaciones = async () => {
        try {
            setProcesando(true);
            const res = await reclasificacionesHTP(infoProyecto?.id_obra_principal);
    ...
    const handleClickDeductivas = async (row:any) => {
        try {
            setProcesando(true);
            const res = await getDetalleDeductivasHTTP(row?.contrato);
    ...
    const handleClickTablasOene = async (row:any, dataOneClickDataTipo_:any)=> {
        window.open(`http://201.132.19.132/dirac/controller/redirect.php?e=1&obra=${row?.contrato}&usuario=${props?.usuario}`);
    }
                            ...
                            <Grid item md={12} sm={12} xl={12} xs={12}>
                                <Grid container>
                                    ...
                                    <Grid item xs={12} style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingTop: 5, cursor: 'pointer' }} onClick={() => { handleReclasificaciones() }} >
                                        ...
                                        <p style={{ margin: 0, color: darkMode ? 'white' : '#576e85' }} className='inicio-new-dashboard-sub-titulo'><strong>Reclasificación:</strong> {numericFormatter(reclasificacion + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' })}</p>
                                    </Grid>
                                    <Grid item xs={12} style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingTop: 5, cursor: 'pointer' }} onClick={() => { handleOrdenCompra() }} >
                                        ...
                                        <p style={{ margin: 0, color: darkMode ? 'white' : '#576e85' }} className='inicio-new-dashboard-sub-titulo'><strong>Órdenes de compra:</strong> {numericFormatter(ordenesCompra + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' })}</p>
                                    </Grid>
                                    ...
                                    <Grid item xs={12} style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingTop: 5, cursor: 'pointer' }} onClick={() => { handleAhorro() }}>
                                        <Avatar alt="Usuario" src={iconoIrAPM} sx={{ borderRadius: 1, backgroundColor: '#7593b2' }} />
                                        <p style={{ margin: 0, color: darkMode ? 'white' : '#28334a' }} className='inicio-new-dashboard-sub-titulo'><strong>Ahorro del proyecto:</strong> {numericFormatter((props?.data?.total_ahorrado || 0) + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' })} </p>
                                    </Grid>
                                </Grid>
                            </Grid>
                ...
                <Accordion style={{ backgroundColor: '#e9eef1', boxShadow: 'none', border: 'none' }} disabled={props?.procesandoResumen}>
                    ...
                    <AccordionDetails style={{ backgroundColor: darkMode ? '#28334a' : '#e9eef1' }}>
                        ...
                        <Grid container spacing={1} style={{ border: darkMode ? 'solid 2px white' : 'solid 2px #28334a', marginTop: 10, padding: 20 }}>
                            ...
                            <Grid item md={4} sm={12} xl={4} xs={12} style={{ cursor: 'pointer' }} onClick={() => { handleDeductivas() }}>
                                <Grid container>
                            ...
                            <Grid item md={4} sm={12} xl={4} xs={12} >
                                <Grid container>
                                    ...
                                    <Grid xs={12} style={{ borderBottom: 'solid 2px rgb(87, 110, 133)' }}> <p style={{ color: 'rgb(87, 110, 133)' }} className='inicio-new-dashboard-sub-titulo'>{numericFormatter((estimacionesContabilizados + anticiposContabilizados + fondoGarantiaContabilizados) + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' })}</p> </Grid>
                                ...
                                <Card>
                                    <CardContent>
                                        {(props?.data?.proyectos || [])?.length ? <DinamicTableMejorada
                                            ...
                                            enAccion={(accion, row)=>{
                                                if(accion === 'ir_apm'){
                                                    window.open(`http://201.132.19.132/dirac/controller/redirect.php?e=1&obra=${row?.id_contrato}&usuario=${props?.usuario}`);
                                                }
                                                if(accion === 'detalle'){
                                                    navigate('/dashboard-apm-contrato'+'?usuario='+usuario+'&contrato='+contrato+'&id_contrato='+row?.id_contrato);
                                                }
                                            }}
                                            esAvanceConGrafica
                        ...
                        <DinamicTableMejorada
                            ...
                            data={dataDeductivas.map((r: any) => {
                                return {
                                    ...
                                    deductivas: numericFormatter((r?.deductivas || 0) + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' }),
                                }
                            })}
                        />
                    </Grid> : null}
                        ...
                        flex
                            ...
                            data={dataFondoGarantia.map((r: any) => {
                                return {
                                    ...
                                    fondo_grantia: numericFormatter((r?.fondo_grantia || 0) + '', { thousandSeparator: ',', decimalScale: 2, fixedDecimalScale: true, prefix: '$' }),
                                }
                            })}
                        />
                    </Grid> : null}
...
export default InicioNewDashboard;