...
interface HeaderNewDashboardProps {
    enMenu: () => void
    ocultaMenu: () => void
}
...
const HeaderNewDashboard: React.FC<HeaderNewDashboardProps> = (props: HeaderNewDashboardProps) => {
    ...
    const contratosVencidos = useSelector((state: any) => state?.app?.new_dashboard?.cuentaContratosVencidos || []);
    const contratosProxVencer = useSelector((state: any) => state?.app?.new_dashboard?.cuentaContratosProximosVencer || []);
        ...
        <>
                        ...
                        <Box sx={{ alignItems: 'center', ml: 2, mr: 2, display: { xs: "none", md: "flex" } }}>
                            <Grid container style={{ textAlign: 'right', marginLeft: 5 }}>
                                <Grid item xs={4} style={{}}>
                                    {/* Notificaciones */}
                                    <HomeIcon  fontSize='large' style={{ width: '100px', position: 'relative', left: '25px', cursor: 'pointer' }} onClick={() => {
                                          navigate('/')
                                    }} />
                                </Grid>
                                <Grid item xs={4} style={{}}>
                                    {/* Notificaciones */}
                                    <Badge badgeContent={contratosVencidos?.length} color="primary"  >
                                ...
                                <Grid item xs={4} style={{}}>
                                    {/* Configuración */}
                                    <Badge badgeContent={contratosProxVencer?.length} color="warning">
                            ...
                            <Grid container style={{ textAlign: 'right', marginLeft: 5 }}>
                                <Grid item xs={12} style={{}}>
                                    <p style={{ fontSize: '13px', margin: 0, padding: 0 }} className='aqui-mi-titulo'>{nombreProyecto?.obra || ''}</p>
                            ...
                            {/* Imagen del usuario */}
                            <Avatar alt="Usuario" src="/ruta/a/imagen-de-usuario.jpg" sx={{ ml: 2 }} style={{ borderRadius: 3, border: 'solid 1px #222b3a' }} />
                        </Box>
                    </Toolbar>
                </Box>
            </AppBar>
            ...
            <ModalComponent handleClose={handleisAlerClose} isOpen={isAlertOpen} key={'alerta'}>
                <Grid container spacing={2} style={{ textAlign: 'center' }}>
                    <Grid item xs={12}>
                        {esVencidos ? (contratosVencidos?.length ? <DinamicTable
                            key={'Carpetas'}
                            data={contratosVencidos}
                        /> : <p>Sin resultados</p>) : (contratosProxVencer?.length ? <DinamicTable
                            key={'Carpetas'}
                            data={contratosProxVencer}
                        /> : <p>Sin resultados</p>)}
                    </Grid>
                </Grid>
            </ModalComponent>
...
export default HeaderNewDashboard;
