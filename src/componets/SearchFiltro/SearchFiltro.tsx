...
const SearchFiltro: React.FC<SearchFiltroProps> = (props:SearchFiltroProps) => {
    ...
    const {
        ...
        texto,
        setTexto
    } = useSearchFiltro(props);

    return (
        <Grid container spacing={2}>
            <Grid item xs={12} className='bordersContainers' style={darkMode ? { backgroundColor: props?.bg ? props?.bg : '#1f283e', padding: '10px' } : { backgroundColor: props?.bg ? props?.bg : '#fff', padding: '10px' }}>
